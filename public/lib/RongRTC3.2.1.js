...
(function (global, factory) {
  ...
  var Storage = (function () {
    ...
    function isKeyExist(key) {
      // do not depend on value cause of '鍜�0
      return (
    ...
    function get$$1(key) {
      ...
      var val =
        localStorage.getItem(tempKey) || sessionStorage.getItem(tempKey);
      val = JSON.parse(val);
      if (
        val !== null &&
  ...
  var getErrors = function getErrors() {
    var errors = [
      ...
      },
      {
        ...
        msg:
          "The roomId is illegal and can contain only upper and lower case letters, Arabic numerals, +, =, -, _ and cannot exceed 64 characters in length",
      },
      {
        code: 50002,
      ...
      },
      {
        ...
        msg:
          "Stream not exist. Please check user.id銆乻tream.type or stream.tag",
      },
      {
        code: 50030,
      ...
      },
      {
        ...
        msg:
          "Track not exist. Please check user.id銆乻tream.type or stream.tag",
      },
      {
        code: 50031,
      ...
      },
      {
        ...
        msg:
          "IM SDK version is too low, minimum version 2.4.0, please check: https://www.rongcloud.cn/docs/web_rtclib.html",
      },
      {
        code: 50054,
      ...
      },
      {
        code: 50065,
        name: "ROOM_USER_KICK",
        msg: "You have been removed from the room锛�",
      },
      {
        code: 50066,
        name: "ROOM_USER_BLOCK",
        msg: "You are not allowed to join the room锛�",
      },
      {
        code: 40001,
        name: "NOT_IN_ROOM",
  ...
  var REQUEST_TIMEOUT = 10 * 1000; // 榛樿鏀逛负 10s
  ...
  var LIVE_LAYOUT_MODE = {
    CUSTOMIZE: 1, // 鑷畾涔夊竷灞€
    SUSPENSION: 2, // 鎮诞
    ADAPTATION: 3, // 鑷€傚簲甯冨眬
  };

  /* 鐩存挱鑷畾涔夊竷灞€, 瑙嗛娓叉煋鏂瑰紡 */
  ...
  /* 鐩存挱璁剧疆(鑷畾涔夊竷灞€璁剧疆, 璇锋眰 mediaServer 鏃朵紶鍏�) */
  var LIVE_CONFIG = {
  ...
  var STAT_TPL = {
    R1:
      "R1\t{rtcVersion}\t{imVersion}\t{platform}\t{pcName}\t{pcVersion}\t{browserName}\t{browserVersion}\t{deviceId}",

    R2: "R2\t{type}\t{state}\t{deviceId}\r{trackIds}",

    R3_ITEM:
      "{googTrackId}\t{googCodecName}\t{audioLevel}\t{samplingRate}\t{trackSent}\t{packLostSentRate}\t{frameRate}\t{resolution}\t{googRenderDelayMs}\t{googJitterSent}\t{googNacksSent}\t{googPlisSent}\t{googRtt}\t{googFirsSent}\t{codecImplementationName}\t{trackState}",
    R3:
      "R3\t{totalRate}\t-1\t-1\t-1\t{networkType}\t{rtt}\t{localAddress}\t{receiveBand}\t{sendBand}\t{packetsLost}\t{deviceId}\r{tracks}",
    R3_KEYS: [
      "mediaType",
      "googTrackId",
      "googCodecName",
    ...
    R4_ITEM:
      "{googTrackId}\t{googCodecName}\t{audioLevel}\t{samplingRate}\t{trackReceived}\t{packLostReceivedRate}\t{frameRate}\t{resolution}\t{googRenderDelayMs}\t{googJitterReceived}\t{googNacksReceived}\t{googPlisReceived}\t{googRtt}\t{googFirsReceived}\t{codecImplementationName}\t{trackState}",
    R4:
      "R4\t{totalRate}\t-1\t-1\t-1\t{networkType}\t{rtt}\t{localAddress}\t{receiveBand}\t{sendBand}\t{packetsLost}\t{deviceId}\r{tracks}",
    R4_KEYS: [
      "mediaType",
      "googTrackId",
      "googCodecName",
  ...
  var formatLiveConfig = function formatLiveConfig(config) {
    ...
    var customLayout = utils.handleObjKeys(
      config.customLayout,
      utils.humpToLine
    ); // 椹煎嘲杞寲涓轰笅鍒掔嚎
  ...
  /* 
      1. http 涓�, 鍏ㄩ儴璁や负鏈夋晥
      2. https 涓�, 娌℃湁鍗忚澶�(璇锋眰鏃朵細鑷姩鎷兼帴)鎴栧崗璁ご涓� https 璁や负鏈夋晥
     */
    ...
    createClass(Stream, [
      ...
      },
      {
        ...
        value: function setMixConfig(config) {
          ...
          var isLiveMode = mode === RTC_MODE.LIVE; // 鐩存挱妯″紡
          var isAnchor = liveRole === LIVE_ROLE.ANCHOR; // 涓绘挱

          if (!isLiveMode) {
            // 闈炵洿鎾ā寮�
            return utils.Defer.reject(
              ErrorType.Inner.SET_LIVE_CONFIG_MODE_ERROR
            );
          }
          if (!isAnchor) {
            // 闈炰富鎾�
            return utils.Defer.reject(
              ErrorType.Inner.SET_LIVE_CONFIG_ROLE_ERROR
            );
          }
  ...
  var Path = {
    ...
    LIVE_SUBSCRIBE: "/broadcast/subscribe", // 鐩存挱妯″紡, 璁㈤槄鎺ュ彛
    LIVE_CONFIG: "/server/mcu/config", // 鐩存挱妯″紡, 鑷畾涔夋洿鏀归厤缃帴鍙�
    LIVE_EXIT: "/broadcast/exit", // 鐩存挱妯″紡, 閫€鍑烘帴鍙�
    EXIT: "/exit",
  };
  ...
  function request$1() {
    ...
    var setOption = function setOption(_config) {
      ...
      if (!utils.isEmpty(urls)) {
        // 灏� urls 璁剧疆鎻愬嚭, 缁熶竴鍦� setUrls 鍐呮牎楠屾牸寮�
        setUrls(urls);
        delete _config.urls;
      }
        ...
        var doRequest = function doRequest(error) {
          ...
          if (isRange) {
            ...
            if (utils.isUndefined(error)) {
              ...
              error = utils.isEqual(error.status, 0)
                ? Inner.MEDIA_SERVER_ERROR
                : error;
            }
            return reject(error);
          }
  ...
  var globalPC = void 0; // pc 涓哄崟渚�, 鍙粠 PeerConnection.getInstance 鑾峰彇
  ...
  var PeerConnection = (function (_EventEmitter) {
    ...
    function PeerConnection(option) {
      ...
      var _this = possibleConstructorReturn(
        this,
        (
          PeerConnection.__proto__ || Object.getPrototypeOf(PeerConnection)
        ).call(this)
      );
    ...
    createClass(PeerConnection, [
      ...
      },
      {
        ...
        value: function isNegotiate() {
          ...
          return (
            utils.isEqual(state, ICEEvent.FAILED) ||
            utils.isEqual(state, ICEEvent.DISCONNECTED)
          );
        },
      },
      {
  ...
  var LiveStream = (function () {
    function LiveStream(im, option) {
      ...
      self.network = new Network(detect); // 缃戠粶鍡呮帰
      self.SubPromiseCache = {}; // 瑙備紬浠呰兘璁㈤槄涓€涓棰戞祦, 涓旇棰戞祦 id 鏃犺寰�
      self.SubLiveUrlsCache = null; // 鐩墠閫昏緫鍙兘鏈変竴涓� liveUrl
    ...
    createClass(LiveStream, [
      ...
      },
      {
        ...
        value: function setPC() {
          var self = this;
          // 濡傛灉宸插垱寤� peerconnection, 鍒欎笉鍐嶅鐞�
      ...
      },
      {
        ...
        value: function subscribe(room, callback) {
          ...
          return utils.deferred(function (resolve, reject) {
            return self.getRTCToken().then(
              function (result) {
                var rtcToken = result.rtcToken;

                self.SubPromiseCache = {
                  resolve: resolve,
                  reject: reject,
                };
  ...
  var IM = (function (_EventEmitter) {
    ...
    function IM(option) {
      ...
      var _this = possibleConstructorReturn(
        this,
        (IM.__proto__ || Object.getPrototypeOf(IM)).call(this)
      );
        ...
        utils.forEach(users, function (user) {
          ...
          switch (+state) {
            case UserState.JOINED:
              context.emit(DownEvent.ROOM_USER_JOINED, {
                id: id,
              });
              break;
            case UserState.LEFT:
            case UserState.OFFLINE:
              ...
              context.emit(DownEvent.ROOM_USER_LEFT, {
                id: id,
              });
              break;
      ...
      var renameMessage = function renameMessage(message) {
        ...
        var isCustom = utils.isEqual(
          im.MessageType.UnknownMessage,
          messageType
        );
      ...
      im.messageWatch(function (message) {
        ...
        switch (type) {
          case Message.STATE:
            ...
            break;
          case Message.KICK:
            context.emit(DownEvent.ROOM_USER_KICK, {
              msg: ErrorType.Inner.ROOM_USER_KICK,
            });
            break;
          case Message.PUBLISH:
            user = {
    ...
    createClass(IM, [
      ...
      },
      {
        ...
        value: function getRequestTimeout() {
          ...
          if (timeOut && utils.isNumber(timeOut)) {
            timeOut = timeOut * 1000; // navi 涓嬪彂鍗曚綅涓� s
          } else {
            timeOut = REQUEST_TIMEOUT;
          }
          return timeOut;
      ...
      },
      {
        ...
        value: function removeUserData(keys, isInner, message) {
          ...
          return utils.deferred(function (resolve, reject) {
            im.getInstance().removeRTCUserData(
              id,
              keys,
              isInner,
              {
      ...
      },
      {
        ...
        value: function removeRoomData(keys, isInner, message) {
          ...
          return utils.deferred(function (resolve, reject) {
            im.getInstance().removeRTCRoomData(
              id,
              keys,
              isInner,
              {
  ...
  var DefaultStream = (function () {
    function DefaultStream(im, option) {
      ...
      self.StreamCache = utils.Cache(); // 缂撳瓨宸茶闃� MediaStream, 瑙勫垯 userId_type: mediaStream, 鏂逛究瑙嗛娴佹搷浣�
      self.DataCache = utils.Cache();
      self.SubPromiseCache = utils.Cache(); // 缂撳瓨璁㈤槄 Promise
      self.PubResourceCache = utils.Cache(); // 缂撳瓨鍙戝竷璧勬簮
      self.PublishStreamCache = utils.Cache(); // 缂撳瓨鑷繁鍙戝竷鐨勮棰戞祦

      self.subCache = utils.Cache(); // 缂撳瓨璁㈤槄鍏崇郴锛屾瘡娆′慨鏀归渶鍚屾鍏ㄩ噺鏁版嵁. userId: [{ streamId: '', uri: '', type: 1, tag: ''}]
    ...
    createClass(DefaultStream, [
      ...
      },
      {
        ...
        value: function getUsersById(user) {
          ...
          utils.forEach(streams, function (tag, msid) {
            var types = msTypes[msid] || [];
            var type = msTypes[0];
            type = utils.isEqual(types.length, 2)
      ...
      },
      {
        ...
        value: function getStreamUser(stream) {
          ...
          if (hasUnderline) {
            ...
            var _pc$getStreamSymbolBy2 = slicedToArray(
              _pc$getStreamSymbolBy,
              2
            );
        ...
        value: function getBody(desc) {
          ...
          var ext = (function (resolution) {
            ...
            for (var key in resolution) {
              var item = {
                trackId: resolution[key][0].videoTrackId,
              };
      ...
      },
      {
        ...
        /* 鑾峰彇娴� track 鐨勫彲鐢ㄧ姸鎬�, 鏍煎紡涓�: { video: false, audio: true }. 鍙叡鐢� */
      },
      {
        key: "getTrackState",
          ...
          var update = function update(_uris) {
            utils.forEach(states, function (_ref4, streamId) {
              ...
              utils.map(_uris, function (uri) {
                ...
                if (
                  isSameStream &&
                  utils.isEqual(uri.mediaType, StreamType.VIDEO)
                ) {
                  utils.extend(uri, {
                    state: video,
                  });
                ...
                if (
                  isSameStream &&
                  utils.isEqual(uri.mediaType, StreamType.AUDIO)
                ) {
                  utils.extend(uri, {
                    state: audio,
                  });
        ...
        value: function usersHandler(users) {
          ...
          utils.forEach(users, function (data, id) {
            ...
            if (utils.isEqual(currentUserId, id)) {
              ...
              type = utils.isEqual(uris.length, 1)
                ? type
                : StreamType.AUDIO_AND_VIDEO;
              var currentUser = {
                id: id,
              ...
              return self.unpublish(currentUser); // 浼氳鍐呭凡鏈夎嚜宸�, 鍏� unpublish
            }
            utils.forEach(uris, function (uri) {
      ...
      },
      {
        ...
        value: function compare() {
          ...
          var compareStreams = function compareStreams(
            localUsers,
            remoteUsers
          ) {
            ...
            utils.forEach(remoteUsers, function (remoteUris, remoteMSId) {
              ...
              var _pc$getStreamSymbolBy3 = pc.getStreamSymbolById(remoteMSId),
                _pc$getStreamSymbolBy4 = slicedToArray(
                  _pc$getStreamSymbolBy3,
                  1
                ),
            ...
            utils.forEach(tempLocalUsers, function (localUris, localMSId) {
              var _pc$getStreamSymbolBy5 = pc.getStreamSymbolById(localMSId),
                _pc$getStreamSymbolBy6 = slicedToArray(
                  _pc$getStreamSymbolBy5,
                  1
                ),
          ...
          var compareUser = function compareUser(localUsers, remoteUsers) {
            ...
            utils.forEach(tempLocalUsers, function (_ref9) {
              ...
              im.emit(DownEvent.ROOM_USER_LEFT, {
                id: id,
              });
            });
          };
                ...
                return request$2
                  ...
                  .then(
                    function (response) {
                      ...
                      urls.configUrl &&
                        request$2.setOption({
                          mcuUrls: [getMCUConfigUrl(urls.configUrl)], // 鐩墠 server 瑕佹眰鍔犲叆 8080
                        });
                      resolve(result);
                    },
                    function (error) {
        ...
        value: function subscribe(user, callback) {
          ...
          utils.forEach(types, function (type) {
            ...
            utils.forEach(subs, function (sub) {
              ...
              var isExist =
                utils.isEqual(type, existType) && utils.isEqual(tag, existTag);
              if (isExist) {
                isAdd = false;
              }
            });
          ...
          if (
            SubscribeCache.getKeys().length > 0 ||
            self.PublishStreamCache.getKeys().length > 0
          ) {
      ...
      },
      {
        ...
        value: function setMixConfig(config) {
          ...
          if (utils.isEmpty(domains)) {
            return utils.Defer.reject(
              ErrorType.Inner.MUST_PUBLISHED_BEFORE_SETMIXCONFIG
            );
          }
  ...
  function StreamHandler(im, option, rongRTC) {
    ...
    /**
     * 鍖哄垎鐩存挱杩樻槸鏅€氶煶瑙嗛
     * 娉ㄦ剰: 鐩存挱鐨勪富鎾鎵€鏈夋祦绋嬩笌鏅€氶煶瑙嗛鐩稿悓, 渚濈劧璧� DefaultStream
     */
      ...
      prosumer.consume(function (_ref, next) {
        ...
        switch (event) {
          ...
          case UpEvent.STREAM_UNPUBLISH:
            return (_stream2 = stream).unpublish
          ...
          case UpEvent.STREAM_SUBSCRIBE:
            return (_stream3 = stream).subscribe
              .apply(
                _stream3,
          ...
          case UpEvent.STREAM_UNSUBSCRIBE:
            return (_stream4 = stream).unsubscribe
              .apply(_stream4, toConsumableArray(args))
              .then(function (result) {
                next();
                resolve(result);
              })
          ...
          case UpEvent.VIDEO_DISABLE:
            return (_stream9 = stream).disable
          ...
          case UpEvent.VIDEO_ENABLE:
            return (_stream10 = stream).enable
          ...
          case UpEvent.LIVE_CONFIG:
            return (_stream11 = stream).setMixConfig
  ...
  function Stat(im, option) {
    ...
    var getR3Object = function getR3Object(ssrc) {
      ...
      try {
        for (
          var _iterator = STAT_TPL.R3_KEYS[Symbol.iterator](), _step;
    ...
    var getR4Object = function getR4Object(ssrc) {
      ...
      try {
        for (
          var _iterator2 = STAT_TPL.R4_KEYS[Symbol.iterator](), _step2;
    ...
    var format = function format(stats) {
      ...
      var getRate = function getRate(ssrc) {
        ...
        // 鍙戦€併€佹帴鏀舵€荤爜鐜囦负绌猴紝鐩存帴杩斿洖锛屼笅娆℃湁鍚堟硶鍊煎啀琛岃绠�
      ...
      var getTrack = function getTrack(stat) {
        var track = {};
        var audioLevel =
          stat["audioOutputLevel"] || stat["audioInputLevel"] || STAT_NONE;
        var frameRate =
          stat["googFrameRateInput"] ||
          stat["googFrameRateOutput"] ||
          STAT_NONE;
        ...
        var calcLostRate = function calcLostRate(stat) {
          ...
          var calcHandles = {
            sender: function sender() {
              var prePacketsSent = StatCache.get(
                StatCacheName.PACKAGE_SENT_ENUM[_mediaType]
              );
              StatCache.set(
                StatCacheName.PACKAGE_SENT_ENUM[_mediaType],
                packetsSent
              ...
              var prePacketsLostSent = StatCache.get(
                StatCacheName.PACKAGE_SENT_LOST_ENUM[_mediaType]
              );
              StatCache.set(
                StatCacheName.PACKAGE_SENT_LOST_ENUM[_mediaType],
                packetsLost
            ...
            receiver: function receiver() {
              var prePacketsReceived = StatCache.get(
                StatCacheName.PACKAGE_RECEIVED_ENUM[_mediaType]
              );
              StatCache.set(
                StatCacheName.PACKAGE_RECEIVED_ENUM[_mediaType],
                packetsReceived
              ...
              var prePacketsLostReceived = StatCache.get(
                StatCacheName.PACKAGE_RECEIVED_LOST_ENUM[_mediaType]
              );
              StatCache.set(
                StatCacheName.PACKAGE_RECEIVED_LOST_ENUM[_mediaType],
                packetsLost
      ...
      var getPair = function getPair(pair) {
        ...
        if (preBytesSent) {
          totalSend =
            ((_bytesSent - preBytesSent) * 8) / 1024 / (frequency / 1000);
        }
        if (preBytesReceived) {
          totalReceive =
            ((_bytesReceived - preBytesReceived) * 8) /
            1024 /
            (frequency / 1000);
        }
          ...
          function (require, module, exports) {
            ...
            function shimGetSendersWithDtmf(window) {
              ...
              if (
                ...
                var shimSenderWithDtmf = function shimSenderWithDtmf(
                  pc,
                  track
                ) {
                  return {
          ...
          function (require, module, exports) {
            ...
            function shimGetUserMedia(window) {
              ...
              var constraintsToChrome_ = function constraintsToChrome_(c) {
                if (
                  (typeof c === "undefined" ? "undefined" : _typeof$$1(c)) !==
                    "object" ||
                  c.mandatory ||
                  c.optional
                ) {
        ...
        6: [
          function (require, module, exports) {
            ...
            function shimRTCIceCandidate(window) {
              ...
              utils.wrapPeerConnectionEvent(window, "icecandidate", function (
                e
              ) {
                if (e.candidate) {
            ...
            function shimSendThrowTypeError(window) {
              ...
              utils.wrapPeerConnectionEvent(window, "datachannel", function (
                e
              ) {
          ...
          function (require, module, exports) {
            ...
            function getCommonCapabilities(
              ...
              var rtxCapabilityMatches = function rtxCapabilityMatches(
                ...
                var rCodec = findCodecByPayloadType(
                  rRtx.parameters.apt,
                  rCodecs
                );
                return (
            ...
            module.exports = function (window, edgeVersion) {
              ...
              RTCPeerConnection.prototype.setRemoteDescription = function (
                ...
                var isIceLite =
                  SDPUtils.matchPrefix(sessionpart, "a=ice-lite").length > 0;
                var usingBundle =
                  SDPUtils.matchPrefix(sessionpart, "a=group:BUNDLE ").length >
                  0;
  ...
  var Client = (function (_EventEmitter) {
    ...
    function Client(option, rongRTC) {
      ...
      var _this = possibleConstructorReturn(
        this,
        (Client.__proto__ || Object.getPrototypeOf(Client)).call(this)
      );
      ...
      // 绂诲紑鎴块棿鏃�, 闇€灏� client 缁戝畾鐨� RoomEvents 浜嬩欢鍙栨秷, 鍚﹀垯閲嶆柊鍔犲叆鎴块棿浼氶噸澶嶈Е鍙戝墠鍚庡娆＄殑鐩戝惉
    ...
    createClass(Client, [
      {
        ...
        value: function exec(params) {
          ...
          var AudienceEvents = [
            UpEvent.STREAM_SUBSCRIBE,
            UpEvent.STREAM_UNSUBSCRIBE,
          ];
  ...
  var RongRTC = (function () {
    function RongRTC(_option) {
      ...
      utils.forEach(
        [Room, Stream, Storage$2, Message$1, Device, Report, Monitor],
        function (module) {
